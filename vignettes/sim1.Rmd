...
5 models are fit to each data set: classical, oracle, ROAMS, Huber, and trimmed. Each data frame of models is up to a gigabyte in size and takes days to fit, so they are saved in 5 separate `.rds` files.
  ...
  mutate(
    classical_model = furrr::future_map(y, function(y) {
      ...
      classical_SSM(
        ...
        build = build, 
        lower = c(0, 1e-12, 1e-12, 1e-12, 1e-12), 
...
Using the model fits, in-sample and out-of-sample evaluation metrics (sensitivity, specificity, estimation RMSE, one-step-ahead MSFE) are computed. Functions to compute these metrics are sourced from the `evaluation_functions.R` script file. This process takes a few minutes to run, so the output is saved to the file `eval_metrics_study1.rds`. 

```{r eval = run_sim}
classical_study1 = read_rds(paste0(data_path, "classical_study1_seed2025.rds"))
roams_study1 = read_rds(paste0(data_path, "roams_study1_seed2025.rds"))
oracle_study1 = read_rds(paste0(data_path, "oracle_study1_seed2025.rds"))
huber_study1 = read_rds(paste0(data_path, "huber_study1_seed2025.rds"))
trimmed_study1 = read_rds(paste0(data_path, "trimmed_study1_seed2025.rds"))

data_study1 = read_rds(paste0(data_path, "data_study1_seed2025.rds"))
...
We illustrate the results of the `eval_metrics_study1.rds` object using plots and tables.

Load packages for plots and tables:

```{r}
library(patchwork)
# library(tidyplots) #hard coded the two instances this was needed
library(ggtext)
library(knitr)
library(kableExtra)
library(ggbeeswarm)
...
Check number of replicates per sample size $n$ and outlier configuration:
...
p1 = eval_metrics_study1 |> 
  ...
  # colors_discrete_friendly[1:4]
  scale_colour_manual(values = c("#0072B2","#56B4E9","#009E73","#F5C710")) +
...
p2 = eval_metrics_study1 |> 
  ...
  mutate(facet_label = "Mean Sensitivity (multi-level outliers)") |> 
  ggplot() +
  ...
  # colors_continuous_inferno[c(70,150,230)]
  scale_colour_manual(values = c("#5B126EFF","#D24644FF","#FAC62DFF")) +
  labs(x = "n", y = NULL, 
...
Sensitivity and specificity of the ROAMS method for each data set are plotted. Each point corresponds to a data set: 
...
Mean-aggregated (Figure 4 of @shankar2025) and raw box plots of the one-step-ahead out-of-sample MSFE:
...
eval_metrics_study1 |> 
  ...
  scale_linetype_manual(values = c(
    "solid", "solid", "dotted", "dashed", "dashed", "dotdash" 
  ),
  labels = c(
      "**ROAMS-FUT**",
      "**ROAMS-Kalman**",
...
eval_metrics_study1 |> 
  ...
  scale_linetype_manual(values = c(
    "solid", "dotted", "dashed", "dashed", "dotdash" 
  ),
  labels = c(
      "**ROAMS**",
      "classical",
...
### Table of parameter estimation mean RMSEs for $n$ = 500
...
((I want this to be n = 1000 in the final version))
