...
Simulated data is generated using the `simulate_data_study1()` function. We set it to generate `B = 200` data sets for each combination of sample size (100, 200, 500, 1000) and outlier configuration (clean data, fixed distance, multi-level, cluster). Seed is set to 1303.

```{r settings, message = FALSE, eval = FALSE}
...
```{r classical_model, message = FALSE, eval = FALSE}
...
```{r oracle_model, message = FALSE, eval = FALSE}
plan(multisession, workers = num_cores)
...
```{r robust_ipod, message = FALSE, eval = FALSE}
plan(multisession, workers = num_cores)
...
```{r huber_model, message = FALSE, eval = FALSE}
plan(multisession, workers = num_cores)
...
```{r trimmed_model, message = FALSE, eval = FALSE}
plan(multisession, workers = num_cores)
...
```{r eval = FALSE}
classical_study1 = read_rds(paste0(data_path, "classical_study1_seed1303.rds"))
roams_study1 = read_rds(paste0(data_path, "roams_study1_seed1303.rds"))
oracle_study1 = read_rds(paste0(data_path, "oracle_study1_seed1303.rds"))
huber_study1 = read_rds(paste0(data_path, "huber_study1_seed1303.rds"))
trimmed_study1 = read_rds(paste0(data_path, "trimmed_study1_seed1303.rds"))

data_study1 = read_rds(paste0(data_path, "data_study1_seed1303.rds"))
...
p1 = eval_metrics_study1 |> 
  ...
  scale_colour_manual(values = colors_discrete_friendly[1:4]) +
...
p2 = eval_metrics_study1 |> 
  ...
  scale_colour_manual(values = c(colors_continuous_inferno[80],
                                 colors_continuous_inferno[150],
                                 colors_continuous_inferno[220])) +
  labs(x = "n", y = NULL, 
...
eval_metrics_study1 |> 
  ...
  geom_beeswarm(corral = "random", corral.width = 0.9, alpha = 0.5,
                cex = 0.5) +
  facet_wrap(~ level) +
  scale_colour_manual(values = c(colors_continuous_inferno[70],
                                 colors_continuous_inferno[150],
                                 colors_continuous_inferno[230])) +
