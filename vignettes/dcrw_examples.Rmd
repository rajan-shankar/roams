...
The MSFEs are reported relative to the classical method.
...
results |> 
  ...
  # for paper
  # filter(method %in% c("classical", "roams_fut", "roams_kalman")) |>  
  # group_by(name) |>
  ...
  # ungroup() |> 
  # mutate(
  #   phi_estimate = as.character(
  #     format(round(phi_estimate, 2), nsmall = 2)),
  #   phi_estimate = case_when(
  #   method == "roams_fut" ~ paste0("\\multirow{2}{*}{", phi_estimate, "}"),
  #   method == "roams_kalman" ~ "",
  #   TRUE ~ phi_estimate
  # )) |> 
...
This section creates Figure 6 in @shankar2025.

### Map 

Fit models to blue whale data:
...
p1 = ggplot(plot_data) +
  ...
  #             filter(timepoint %in% c(8, 106, 43, 63)),
  #           aes(x = x_data, y = y_data, label = timepoint),
  #           nudge_y = 1) +
