...
```{r}
results |> 
  select(name, y_oos, oos_outliers_flagged, 
         classical_oos:trimmed_oos) |> 
  rename(y_test = y_oos) |> 
  pivot_longer(cols = ends_with("_oos"), 
               names_to = "method", values_to = "oos_output") |> 
  mutate(
    MSFE = map2_dbl(
      oos_output, y_test,
  ...
  select(name, method, MSFE, MSFE_clean) |> 
  group_by(name) |> 
  mutate(
    MSFE = MSFE / MSFE[which(method == "classical_oos")],
    MSFE_clean = MSFE_clean / MSFE_clean[which(method == "classical_oos")]) |>
