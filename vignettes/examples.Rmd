...
This page shows how the examples in section 3.1 of \cite{shankar2025} are generated.

These examples all use the first-differenced correlated random walk (DCRW) model, a type of linear Gaussian SSM. This model is detailed in \cite{auger-methe_guide_2021}. The DCRW model has the following observation and state processes,
...
\begin{align*}
    \bSigma_\mathbf{v} &= \left[\begin{matrix}\sigma^2_{\mathbf{v},\text{lon}} & 0 \\ 0 & \sigma^2_{\mathbf{v},\text{lat}} \end{matrix}\right].
\end{align*}
A similar matrix is used for the state error variance matrix $\bSigma_\mathbf{w}$.
$$
\begin{align}
\left[
\begin{matrix}
